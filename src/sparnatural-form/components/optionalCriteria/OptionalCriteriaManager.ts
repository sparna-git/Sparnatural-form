...
class OptionalCriteriaManager {
  ...
  constructor(
    ...
    private formVariables: string[], // The list of variables from the form config
  ) {
  ...
  /**
   * Supprime les flags optional sur la branche et ses ancêtres.
   * UNIQUEMENT si la variable fait partie du formulaire (remplie par l'utilisateur).
   * Si la variable n'est pas dans le formulaire (criteria pré-remplie structurelle),
   * on ne touche pas aux optional pour préserver les blocs OPTIONAL dans le SPARQL.
   */
